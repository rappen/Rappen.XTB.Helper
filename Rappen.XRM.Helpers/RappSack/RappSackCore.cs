...
namespace Rappen.XRM.Helpers.RappSack
{
    public abstract class RappSackCore : IOrganizationService
    {
        ...
        public Entity RetrieveFirst(QueryBase query) => RetrieveMultiple(query).Entities.FirstOrDefault();
        ...
        public IEnumerable<T> RetrieveAll<T>(QueryBase query) where T : Entity => RetrieveMultipleAll(query).Entities.Select(x => x.ToEntity<T>());

        public T RetrieveFirst<T>(QueryBase query) where T : Entity => RetrieveMultiple<T>(query).FirstOrDefault();
        ...
        private Entity GetEnvironmentVariableValue(string variablename)
        {
            ...
            var variablevalues = RetrieveFirst(qe);
