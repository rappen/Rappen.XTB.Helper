...
namespace Rappen.XTB.Helpers.Controls
{
    public partial class XRMDataGridView : DataGridView
    {
        ...
        /// </summary>
        /// <param name="addLinks">If true, adds hyperlinks to records.</param>
        /// <param name="fetch">Optional FetchXML to include in the source sheet.</param>
        ...
        public void ExportToExcel(bool addLinks = false, string fetch = null, Action afterExport = null, int? columnMaxWidth = null, bool autoFitRows = false, IEnumerable<string> columns = null, IEnumerable<string> includedColumns = null, IEnumerable<string> excludedColumns = null)
        {
            ...
            try
            {
                ...
                tool.WorkAsync(new WorkAsyncInfo
                {
                    ...
                    Work = (w, a) => ExportToExcelDirect(w, tool.ToolName, fetch, LayoutXML, tool.ConnectionDetail, gridData, columnMaxWidth, autoFitRows),
                    ProgressChanged = p => tool.SetWorkingMessage(p.UserState.ToString()),
                    PostWorkCallBack = a =>
                    {
                        // Restore column visibility
                        RestoreColumnVisibility(originalVisibility);
            ...
            {
                // Restore visibility if something went wrong before async started
                RestoreColumnVisibility(originalVisibility);
                throw;
            }
        }
        ...
        private Dictionary<DataGridViewColumn, bool> ApplyExportColumnVisibility(IEnumerable<string> columns, IEnumerable<string> includedColumns, IEnumerable<string> excludedColumns)
        {
            ...
            var excludeSet = excludedColumns != null
                ? new HashSet<string>(excludedColumns.Select(n => n?.Split('|')[0]).Where(n => n != null), StringComparer.OrdinalIgnoreCase)
                : null;
        ...
        private void RestoreColumnVisibility(Dictionary<DataGridViewColumn, bool> originalVisibility)
        {
            ...
            foreach (var kvp in originalVisibility)
            {
                ...
                {
                    // Ignore errors restoring individual columns
                }
            }
        }

        private PluginControlBase FindParentPluginControlBase()
        {
            Control current = this;
            while (current != null)
            {
                if (current is PluginControlBase pluginControl)
                {
        ...
        private const string ExcelLinkIcon = "🔗";
        ...
        private ExcelGridData CollectGridDataOnUIThread(bool addLinks, ConnectionDetail conndet)
        {
            ...
            void Collect()
            {
                try
                {
                    ...
                    var primaryCol = addLinks ? GetPrimaryNameColumn(visibleCols) : null;
                    var primaryAttr = primaryCol?.Name.Split('|')[0];

                    result = new ExcelGridData
                    {
                        Headers = visibleCols.Select(c => c.HeaderText).ToList(),
                        Rows = new List<List<ExcelCellData>>(),
                        HasLinkColumn = addLinks && primaryCol == null
                    };

                    if (result.HasLinkColumn)
                    {
                        result.Headers.Insert(0, ExcelLinkIcon);
                    }
                    ...
                    for (var r = 0; r < Rows.Count; r++)
                    {
                        ...
                        var entity = addLinks ? GetXRMEntity(r) : null;
                        var urlPrimary = entity?.GetEntityUrl(conndet);
                        var row = new List<ExcelCellData>();

                        if (result.HasLinkColumn)
                        {
                            row.Add(new ExcelCellData { Value = ExcelLinkIcon, Url = urlPrimary });
                        }
        ...
        {
            if (!TryGetPrimaryNameAttribute(out var primaryAttr))
            {
                return null;
            }
        ...
        private bool TryGetPrimaryNameAttribute(out string primaryName)
        {
            primaryName = null;
            if (Service == null || string.IsNullOrWhiteSpace(EntityName))
            {
                return false;
            }

            if (!PrimaryNameCache.TryGetValue(EntityName, out primaryName))
            {
                primaryName = Service.GetEntity(EntityName)?.PrimaryNameAttribute;
                if (!string.IsNullOrWhiteSpace(primaryName))
                {
                    PrimaryNameCache[EntityName] = primaryName;
                }
            }
            return !string.IsNullOrWhiteSpace(primaryName);
        }

        private static void ExportToExcelDirect(BackgroundWorker bw, string toolname, string fetch, string layout, ConnectionDetail conndet, ExcelGridData gridData, int? columnMaxWidth, bool autoFitRows)
        {
            ...
            var sta = new Thread(() =>
            {
                try
                {
                    ...
                    using (new ExcelOptimization(app))
                    {
                        ...
                        TryExcel(() => resultSheet.Activate());
                        TryExcel(() => resultSheet.Range["A1"].Select());
                    }

                    bw?.ReportProgress(95, "Opening Excel...");
                    TryExcel(() => app.Visible = true);
                }
                catch (Exception ex)
                {
                    excelEx = ex;
        ...
        private static void WriteGridToSheet(dynamic sheet, ExcelGridData gridData, BackgroundWorker bw)
        {
        ...
        private static void FormatResultSheet(dynamic sheet, bool hasLinkColumn, int? columnMaxWidth, bool autoFitRows)
        {
            if (hasLinkColumn)
            {
                TryExcel(() => sheet.Columns[1].HorizontalAlignment = XlHAlignCenter);
            }
            ...
            TryExcel(() => sheet.Application.ActiveWindow.SplitRow = 1);
            TryExcel(() => sheet.Application.ActiveWindow.FreezePanes = true);
            TryExcel(() => sheet.UsedRange.VerticalAlignment = XlVAlignTop);
        ...
        private static void PopulateSourceSheet(dynamic wb, string toolname, string fetch, string layout, ConnectionDetail conndet)
        {
            ...
            TryExcel(() => sheet.Move(After: wb.Sheets[wb.Sheets.Count]));
            ...
            TryExcel(() => sheet.Columns[1].AutoFit());
            TryExcel(() => sheet.Columns[2].WrapText = true);
            TryExcel(() => sheet.Columns[2].ColumnWidth = (800 - 5) / 7.0);
            TryExcel(() => sheet.UsedRange.VerticalAlignment = XlVAlignTop);
            TryExcel(() => sheet.UsedRange.Rows.AutoFit());
        }
        ...
        private class ExcelGridData
        {
            ...
            public List<List<ExcelCellData>> Rows { get; set; }
        ...
        private class ExcelOptimization : IDisposable
        {
            ...
            public ExcelOptimization(dynamic app)
            {
                ...
                TryExcel(() => { su = app.ScreenUpdating; app.ScreenUpdating = false; });
                TryExcel(() => { da = app.DisplayAlerts; app.DisplayAlerts = false; });
                TryExcel(() => { ee = app.EnableEvents; app.EnableEvents = false; });
                TryExcel(() => { calc = app.Calculation; app.Calculation = XlCalculationManual; });
                _prevScreenUpdating = su; _prevDisplayAlerts = da; _prevEnableEvents = ee; _prevCalculation = calc;
            }

            public void Dispose()
            {
                TryExcel(() => _app.Calculation = _prevCalculation ?? XlCalculationAutomatic);
                TryExcel(() => _app.EnableEvents = _prevEnableEvents ?? true);
                TryExcel(() => _app.DisplayAlerts = _prevDisplayAlerts ?? true);
                TryExcel(() => _app.ScreenUpdating = _prevScreenUpdating ?? true);
            }
        }
